...
주소 외에는 쓸 수 없다. 클라이언트는 `0.0.0.0/0, ::/0`으로 IPv4와 IPv6를 모두 터널로
보낸다. 인터넷 연결은 IPv4만 지원하며, IPv6는 서버의 IPv4 전용 피어 정책에서 차단된다.
이는 IPv6가 기존 회선으로 우회하는 것을 방지한다. 터널이 꺼졌을 때의 킬 스위치는 아니다.
